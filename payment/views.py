    ...
    def get(self, request, reference):
        ...
        try:
            ...
            if "activate" in reference:
                ...
                transaction = self._process_poll_activation(
                ...
                vote = self._process_vote(
                    transaction, poll, contestant, amount_paid, reference)
            else:
...
class AccountBalanceView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Calculate total earnings from successful vote transactions (60% of vote payments)
            for poll in polls:
                # Get all successful vote transactions for this poll
                ...
                # Sum up the transaction amounts and calculate creator's share (60%)
                poll_earnings = sum(
                    transaction.amount * Decimal('0.6') for transaction in successful_transactions
                )
                total_earnings += poll_earnings

            # Calculate total withdrawals by this creator
...
class PaymentHistoryView(ListAPIView):
    """
    Get payment history for creator-pay poll activations only.
    """
    permission_classes = [IsAuthenticated]
...
class InitiateWithdrawalView(APIView):
    ...
    def post(self, request, poll_id):
        ...
        # Calculate total collected from successful vote transactions for this poll
        successful_transactions = Transaction.objects.filter(
            poll=poll,
            transaction_type='vote',
            success=True
        ...
        total_amount_collected = sum(
            transaction.amount for transaction in successful_transactions
        )

        # Creator gets 60% of total collected for this poll
        max_withdrawable_amount = total_amount_collected * Decimal("0.6")

        # Check if they've already withdrawn for this poll
        already_withdrawn = Withdrawal.objects.filter(
            poll=poll,
            creator=request.user,
            status='successful'
        ).aggregate(total=models.Sum('amount'))['total'] or Decimal('0.0')

        # Available amount is 60% minus what's already been withdrawn
        available_for_withdrawal = max_withdrawable_amount - already_withdrawn
        ...
        if requested_amount > available_for_withdrawal:
            return Response({"error": "Requested amount exceeds the available balance.", "available_for_withdrawal": float(available_for_withdrawal), "already_withdrawn": float(already_withdrawn), }, status=status.HTTP_400_BAD_REQUEST,)
        ...
        try:
            ...
            withdrawal.save()
            logger.error(
                f"Exception during Flutterwave recipient creation: {e}")
...
def paystack_webhook(request):
    ...
    if event == 'charge.success':
        ...
        try:
            ...
            if not transaction:
                logger.error(
                    f"Transaction not found for reference: {reference}")
            ...
            if "activate" in reference:
                ...
                if existing_vote:
                    logger.info(
                        f"Vote already exists for transaction {reference}")
                ...
                if vote_count >= 1:
                    ...
                    transaction.save()
                    logger.info(
                        f"Vote recorded via webhook for poll {poll.id}, contestant {contestant_id}")
                    ...
                    logger.error(
                        f"Insufficient amount for voting: {amount_paid}")
