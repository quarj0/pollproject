...
class PollConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if not poll_exists:
            ...
            raise DenyConnection(f'Poll {self.poll_id} not found')
        ...
        try:
            ...
            poll_data = await database_sync_to_async(
                lambda: PollSerializer(Poll.objects.get(id=self.poll_id)).data
            )()
            votes = await database_sync_to_async(
                lambda: Vote.objects.filter(poll_id=self.poll_id).values('contestant').annotate(
                    total_votes=Sum('number_of_votes')
                ).order_by('-total_votes')
            )()
            poll_data['votes'] = list(votes)
            ...
            logger.info(
                f"WebSocket connected for poll {self.poll_id}: {self.channel_name}")
        except Exception as e:
            logger.error(
                f"Error in WebSocket connect for poll {self.poll_id}: {str(e)}")
    ...
    async def disconnect(self, close_code):
        ...
        try:
            ...
            logger.info(
                f"WebSocket disconnected for poll {self.poll_id}: {self.channel_name}, code: {close_code}")
        except Exception as e:
            logger.error(f"Error in WebSocket disconnection: {str(e)}")
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            logger.error(
                f"JSON decode error for poll {self.poll_id}: {str(e)}")
            await self.send(text_data=json.dumps({
                'type': 'error',
    ...
    async def vote_update(self, event):
        ...
        try:
            ...
            logger.error(
                f"Error in vote update for poll {self.poll_id}: {str(e)}")
            await self.send(text_data=json.dumps({
                'type': 'error',
